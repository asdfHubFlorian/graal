// attempts to relock an errorcheck mutex, relock should return error
...
int main() {
  ...
  if ((result = pthread_mutexattr_settype(&attr, PTHREAD_MUTEX_ERRORCHECK)) !=
      0) {
  ...
  if ((result = pthread_mutex_lock(&mutex)) != 0) {
    printf("pthread_mutex_lock() second time gives return code: %d\n", result);
  }
